use std::ops::{Add, AddAssign, Sub, SubAssign, Mul, MulAssign, Div, DivAssign, Deref, DerefMut, Neg};
use num::Num;
use nalgebra::RealField;

#[derive(Debug, PartialEq, Clone)]
pub struct Polynomial<T: Clone>(pub Vec<T>);

impl <T: Clone> Deref for Polynomial<T> {
    type Target = Vec<T>;
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl <T: Clone> DerefMut for Polynomial<T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.0
    }
}

impl <K: Num + Clone> Polynomial<K> {
    pub fn evaluate(&self, x: K) -> K {
        let mut power = K::one();
        let mut sum = K::zero();
        for a in self.iter() {
            sum = sum + a.clone() * power.clone();
            power = power * x.clone();
        }
        sum
    }

    pub fn derive(&self) -> Polynomial<K> {
        let mut i = K::zero();
        Polynomial(
            self.iter()
                .skip(1)
                .map(|a| { i = i.clone() + K::one(); i.clone() * a.clone() })
                .collect()
        )
    }
}

impl <K: RealField + Clone> Polynomial<K> {
    pub fn roots(&self) -> Vec<K> {
        let zero = K::zero();
        let one = K::one();
        let two = one.clone() + one.clone();
        let four = two.clone() + two.clone();
        match self.as_slice() {
            [] => Vec::new(),
            [_] => Vec::new(),
            [t, m] => vec![(zero - t.clone()) / m.clone()],
            [c, b, a] => {
                let d: K = b.clone().powi(2) - four * a.clone() * c.clone();
                if d.is_sign_negative() {
                    Vec::new()
                } else if d.is_zero() {
                    vec![(zero - b.clone()) / (two * a.clone())]
                } else {
                    let p = zero - b.clone();
                    let q = d.sqrt();
                    let r = two * a.clone();
                    vec![(p.clone() + q.clone()) / r.clone(), (p - q) / r]
                }
            },
            _ => todo!(),
        }
    }
}

macro_rules! impl_componentwise_vector_binary {
    ($Vector:ident, $Trait: ident, $Method:ident, $NegTrait:path, $NegMethod:ident) => {
        impl <'a, K> $Trait<&'a $Vector<K>> for &'a $Vector<K>
            where K: Clone + $Trait<Output=K> + $NegTrait
        {
            type Output = $Vector<K>;
            fn $Method(self, rhs: &'a $Vector<K>) -> Self::Output {
                let mut vec: Vec<K> = self.iter()
                    .zip(rhs.iter())
                    .map(|(x, y)| $Trait::$Method(x.clone(), y.clone()))
                    .collect();

                let len = vec.len();
                for y in &rhs[len..] {
                    vec.push(y.clone().$NegMethod());
                }
                for x in &self[len..] {
                    vec.push(x.clone());
                }

                $Vector(vec)
            }
        }
    }
}
impl_componentwise_vector_binary!(Polynomial, Add, add, Clone, clone);
impl_componentwise_vector_binary!(Polynomial, Sub, sub, Neg<Output=K>, neg);

macro_rules! impl_componentwise_vector_assign {
    ($Vector:ident, $Trait: ident, $Method:ident, $NegTrait:path, $NegMethod:ident) => {
        impl <'a, X> $Trait<&'a $Vector<X>> for $Vector<X>
            where X: Clone + $Trait<X> + $NegTrait
        {
            fn $Method(&mut self, rhs: &'a $Vector<X>) {
                self.iter_mut()
                    .zip(rhs.iter())
                    .for_each(|(x, y)| $Trait::$Method(x, y.clone()));
                for y in &rhs[self.len()..] {
                    self.push(y.clone().$NegMethod());
                }
            }
    }
    }
}
impl_componentwise_vector_assign!(Polynomial, AddAssign, add_assign, Clone, clone);
impl_componentwise_vector_assign!(Polynomial, SubAssign, sub_assign, Neg<Output=X>, neg);

impl <'a, X> Mul<&'a Polynomial<X>> for &'a Polynomial<X>
    where X: Clone + Mul<X, Output=X> + Add<X, Output=X>
{
    type Output = Polynomial<X>;
    fn mul(self, rhs: &'a Polynomial<X>) -> Self::Output {
        let mut vec: Vec<X> = Vec::with_capacity(self.len() + rhs.len() - 1);

        for (i, x) in self.iter().enumerate() {
            for (j, y) in rhs.iter().enumerate() {
                let prod: X = Mul::mul(x.clone(), y.clone());
                if let Some(ij) = vec.get_mut(i + j) {
                    *ij = ij.clone() + prod;
                } else {
                    vec.push(prod);
                }
            }
        }

        Polynomial(vec)
    }
}

impl <'a, X> Neg for &'a Polynomial<X>
    where X: Clone + Neg<Output=X>,
{
    type Output = Polynomial<X>;
    fn neg(self) -> Self::Output {
        Polynomial(
            self.iter()
                .map(|x| Neg::neg(x.clone()))
                .collect()
        )
    }
}

macro_rules! impl_componentwise_scalar_binary {
    ($Vector:ident, $Trait: ident, $Method:ident) => {
        impl <'a, X> $Trait<X> for &'a $Vector<X>
            where X: Clone + $Trait<X, Output=X>,
        {
            type Output = $Vector<X>;
            fn $Method(self, rhs: X) -> Self::Output {
                $Vector(
                    self.iter()
                        .map(|x| $Trait::$Method(x.clone(), rhs.clone()))
                        .collect()
                )
            }
        }
    }
}
impl_componentwise_scalar_binary!(Polynomial, Mul, mul);
impl_componentwise_scalar_binary!(Polynomial, Div, div);

impl <'a, X> MulAssign<X> for Polynomial<X>
    where X: Clone + Mul<X, Output=X>
{
    fn mul_assign(&mut self, rhs: X) {
        for x in self.iter_mut() {
            *x = x.clone() * rhs.clone();
        }
    }
}

impl <'a, X> DivAssign<X> for Polynomial<X>
    where X: Clone + Div<X, Output=X>
{
    fn div_assign(&mut self, rhs: X) {
        for x in self.iter_mut() {
            *x = x.clone() / rhs.clone();
        }
    }
}
